...
LanceDB also builds a resident read snapshot on repeated reads at unchanged table versions, serving anchored reads through compact adjacency. `with_read_snapshot(false)` retains direct scans. Turso bulk loads use prepared bound upserts and ordinary connections allow a lazily filled 1 GiB page cache. These are disclosed memory and execution choices behind the shared API.
