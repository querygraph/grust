...
/// elements, parameters, arithmetic and mixed-type comparisons are not admitted.
...
pub fn lower_expression(
    ...
    lower(expression, variable, schema).map(|(expression, _)| expression)
}
...
fn lower(
    ...
    Ok(match expression {
        ...
        CypherExpr::IsNull { operand, negated } => {
            let (value, _) = lower(operand, variable, schema)?;
            (
                if *negated {
                    value.is_not_null()
                } else {
        ...
        CypherExpr::Unary {
            ...
            let (value, kind) = lower(operand, variable, schema)?;
        ...
        CypherExpr::Binary { op, lhs, rhs } => {
            ...
            let (right, right_type) = lower(rhs, variable, schema)?;
